...
class Click2Datapath < DataPath
  attr_reader :click_command, :click_sock
...
switch[1] #{bs_vlan_encap} -> bs_queue;"
  ...
  def start
    ...
    @app = ExecApp.new(@app_id, self, "#{@click_command}")
  ...
  def update_click_config
    ...
    info("Loading new config to click for vlan #{@vlan_net}:\n#{new_config}")
