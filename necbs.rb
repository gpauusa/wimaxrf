...
class NecBs < Netdev
  ...
  def initialize(dp, auth, bsconfig, asnconfig )
    ...
    @sndPort = asnconfig['asnsndport'] || 54322
     ...
     if !logconfig.nil? then
       ...
       if !logconfig['localoml'].nil? 
         ...
         oml_opts1.merge!(logconfig['localoml'])
         @localinterval = logconfig['localoml']['interval'] || 10
         begin
       ...
       if !logconfig['globaloml'].nil? 
         ...
         oml_opts2.merge!(logconfig['globaloml'])  
         @globalinterval = logconfig['globaloml']['interval'] || 300
         begin
    ...
    debug(snmp_set("iso.0.8802.16.2.1.2.9.1.2.1.6.1", 2610003))
    ...
    @dp.each{ |vl,p|
      info "Starting vlan #{vl} datapath with #{p.length} clients"
      p.start()
    }
...
#    debug("Checking mobile stats...")
#    get_mobile_stats()
#    debug("...found #{@mobs.length} mobiles")
#  end
#  
#  def check_existing()
#   hGREs = {}
...
##         if @clmeasfiel != nil 
##           @clmeasfile << "#{ma},#{mac}," +
##             "#{ulrssi},#{ulcinr},#{m.mcsulrate},#{m.mcsulmod},#{m.tppduul.to_i},#{m.tpsduul.to_i}," +
##             "#{dlrssi},#{dlcinr},#{m.mcsdlrate},#{m.mcsdlmod},#{m.tppdudl.to_i},#{m.tpsdudl.to_i}\n"
##         end
#     
#         if !@localoml.nil? 
#           #debug("#{ma} #{mac} #{ulrssi} #{ulcinr} #{dlrssi} #{dlrssi} #{m.mcsulmod} #{m.mcsdlmod}")
#           ClStat.inject(ma, mac, ulrssi, ulcinr, dlrssi, dlcinr, m.mcsulmod, m.mcsdlmod)
#         end
#        
#     rescue Exception => ex
